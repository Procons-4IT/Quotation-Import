...
#End Region
    ...
    Public Function ValidateFile(ByVal oForm As SAPbouiCOM.Form, ByVal strID As String) As Boolean
        ...
        Try
            ...
            If Path.GetExtension(strPath) <> ".xlsx" Then
                _retVal = False
    ...
    Public Function GetExcelData(ByVal oForm As SAPbouiCOM.Form, ByVal strID As String) As Boolean
        ...
        oRecordSet = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
        Try
            _oDt.TableName = "PQIMPORT"

            _oDt.Columns.Add("DocNum", GetType(String)).Caption = "PQRef"
            _oDt.Columns.Add("LineNum", GetType(String)).Caption = "PQLine"
            _oDt.Columns.Add("ItemCode", GetType(String)).Caption = "ItemCode"
            _oDt.Columns.Add("Dscription", GetType(String)).Caption = "Desc"
            _oDt.Columns.Add("OpenQty", GetType(String)).Caption = "AQuantity"
            _oDt.Columns.Add("UnitMsr", GetType(String)).Caption = "UOM"
            _oDt.Columns.Add("Price", GetType(Double)).Caption = "Price"
            _oDt.Columns.Add("DocCur", GetType(String)).Caption = "Currency"
            _oDt.Columns.Add("Terms", GetType(String)).Caption = "IncoTerms"
            _oDt.Columns.Add("Quantity", GetType(Double)).Caption = "RQuantity"
            _oDt.Columns.Add("ShipDate", GetType(Date)).Caption = "BPD"
            _oDt.Columns.Add("FreeTxt", GetType(String)).Caption = "Remarks"
            _oDt.Columns.Add("Remarks", GetType(String)).Caption = "Import Remarks"

            Dim strPath As String = CType(oForm.Items.Item(strID).Specific, SAPbouiCOM.StaticText).Caption
            If strPath.Length > 0 Then
                If (oApplication.Excel.getExcelTemplate(strPath, _oDt)) Then
                    Dim strDtXML As String = getXMLstring(_oDt)
                    oRecordSet.DoQuery("Exec [Insert_PQImport] '" + strDtXML + "'")
                    _retVal = True
                End If
            End If
    ...
    Public Function importPQData(ByVal oForm As SAPbouiCOM.Form, ByVal strRefNo As String) As Boolean
        Dim oPQuotation As SAPbobsCOM.Documents
        Try
            Dim intStatus As Integer
            Dim intRow As Integer = 0
            Dim oGrid As SAPbouiCOM.Grid = oForm.Items.Item("8").Specific
            Dim blnPriceUpdate As Boolean = False

            oPQuotation = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.oPurchaseQuotations)
            If CType(oForm.Items.Item("15").Specific, SAPbouiCOM.CheckBox).Checked = True Then
                Dim intChoice As Integer = oApplication.SBO_Application.MessageBox("Price Updation is selected. Do you want to update Price?", 1, "Continue", "Cancel Import")
                If intChoice = 2 Then
                    Return False
                ElseIf intChoice = 1 Then
                    blnPriceUpdate = True
                Else
                    Return False
                End If
            Else
                Dim intChoice As Integer = oApplication.SBO_Application.MessageBox("Price Updation is not selected. Do you want to continue without Price Updation?", 1, "Continue", "Cancel Import")
                If intChoice = 2 Then
                    Return False
                ElseIf intChoice = 1 Then
                    blnPriceUpdate = False
                Else
                    Return False
                End If
            End If
            If oPQuotation.GetByKey(strRefNo) Then
                Dim blnStatus As Boolean = False
                If oPQuotation.DocumentStatus = SAPbobsCOM.BoStatus.bost_Open Then
                    For index As Integer = 0 To oGrid.Rows.Count - 1
                        oPQuotation.Lines.SetCurrentLine(oGrid.DataTable.GetValue("LineNum", index) - 1)
                        If oPQuotation.Lines.LineStatus = SAPbobsCOM.BoStatus.bost_Open Then
                            If blnPriceUpdate = True And oGrid.DataTable.GetValue("Price", index) > 0 Then
                                oPQuotation.Lines.UnitPrice = oGrid.DataTable.GetValue("Price", index)
                            End If
                            oPQuotation.Lines.Quantity = oGrid.DataTable.GetValue("Quantity", index)
                            oPQuotation.Lines.ShipDate = oGrid.DataTable.GetValue("ShipDate", index)
                            oPQuotation.Lines.FreeText = oGrid.DataTable.GetValue("FreeTxt", index)
                            blnStatus = True
                        End If
                    Next
                    If blnStatus Then
                        intStatus = oPQuotation.Update()
                        If intStatus <> 0 Then
        ...
        Catch ex As Exception
            Throw ex
        Finally
            System.Runtime.InteropServices.Marshal.ReleaseComObject(oPQuotation)
        End Try
        Return True
    End Function
